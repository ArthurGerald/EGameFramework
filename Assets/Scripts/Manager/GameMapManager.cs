...
public class GameMapManager : IBaseManager
{
    ...
    public void LoadScene(string name, object[] parameters)
    {
        LoadingProgress = 0;
        m_Mono.StartCoroutine(LoadSceneAsync(name));
    //   UIManager.Instance.PopUpWnd(ConStr.LOADINGPANEL, true, new object[] { name, parameters });
    }
    ...
    IEnumerator LoadSceneAsync(string name)
    {
        ...
        AsyncOperation unLoadScene =  SceneManager.LoadSceneAsync("FightScene", LoadSceneMode.Single);
        ...
        if (asyncScene != null && !asyncScene.isDone)
        {
            ...
            EventManager.Instance.Send(EventType.EndChangeScene);
